...
const DocumentManager = ({ currentUser }) => {
    ...
    const [view, setView] = useState('shelves'); // shelves, list, folder, search
    ...
    // Permissions (Admin always has full access via DataContext seed, but good to check)
    const canRead = currentUser.permissions?.read || currentUser.role === 'admin';
    const canWrite = currentUser.permissions?.write || currentUser.role === 'admin';
    const canDelete = currentUser.permissions?.delete || currentUser.role === 'admin';
        ...
        <div>
            ...
            <div className="search-section">
                <div className="search-box">
                    ...
                    />
                    <Button onClick={handleSearch} style={{ width: 'auto' }}>🔍 Axtar</Button>
                </div>
            </div>
                ...
                <form onSubmit={handleCtxAddDoc}>
                    <div className="form-row">
                        ...
                        <div className="form-group">
                            <label>Növ</label>
                            <select value={newDoc.type} onChange={e => setNewDoc({ ...newDoc, type: e.target.value })} required style={{ width: '100%', padding: '12px', border: '1px solid #ddd', borderRadius: '6px' }}>
                    ...
                    <div className="form-row">
                        ...
                        <div className="form-group">
                            ...
                            <select value={newDoc.location} onChange={e => setNewDoc({ ...newDoc, location: e.target.value })} required style={{ width: '100%', padding: '12px', border: '1px solid #ddd', borderRadius: '6px' }}>
                                <option value="">Seçin...</option>
                    ...
                    {newDoc.location && folders[newDoc.location] && folders[newDoc.location].length > 0 && (
                        <div className="form-group">
                            ...
                            <select value={newDoc.folder} onChange={e => setNewDoc({ ...newDoc, folder: e.target.value })} style={{ width: '100%', padding: '12px', border: '1px solid #ddd', borderRadius: '6px' }}>
